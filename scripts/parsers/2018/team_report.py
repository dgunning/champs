...
def list_athletes(team_records):
    ...
    :return:
    """
    athlete_cols = ['Class','Event','Athlete','Round','Rank','Mark','Points']
    ...
    .rename(columns={'Class_x':'Class','Event_x':'Event','Round_x':'Round','Rank_x':'Rank','Mark_x':'Mark','Team_x':'Team','Points_x':'Points'})[athlete_cols].reset_index(drop=True)
    athletes.Class = athletes.Class.fillna('Open')
    print(athletes.columns.values)
    relays = get_relays(team_records)
    athletes= athletes.append(relays[athlete_cols])\
        .drop_duplicates().sort_values(['Class','Event','Round']).reset_index(drop=True).dropna()
    return athletes
...
def team_athletes(champs, team, gender):
    ...
    athlete_records=athlete_records.sort_values(['Class','Event','Athlete','RoundNumber']).reset_index(drop=True).drop(columns=['RoundNumber','Gender'])
    return athlete_records.drop(columns=['Team'])
